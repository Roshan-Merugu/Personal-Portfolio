...
export const Skills = () => {
  return (
    <Section id="stack" index="sec_06" label="Stack" title="The toolchain. Sharpened daily.">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-wire border border-wire">
        {skills.map((group) => (
          <div key={group.category} className="bg-void p-6">
            <div className="flex items-start sm:items-center justify-between mb-6 pb-3 border-b border-wire">
              <h3 className="mono text-xs uppercase tracking-widest text-volt">
                <span className="text-text-mute mr-2">//</span>
                {group.category}
              </h3>
              <span className="mono text-[10px] text-text-mute tabular-nums flex-shrink-0 ml-2">
